    ...
    for uid in members:
        if uid:
            c.execute('INSERT INTO chat_members (chat_id, user_id, joined_at) VALUES (?, ?, ?)',
                      (chat_id, uid, datetime.now().isoformat()))
    db.commit()
    db.close()
...
def view_community(community_id):
    ...
    return render_template('view_community.html', community=community, posts=posts, 
                          members_count=members_count, is_admin=is_admin, is_member=is_member,
...
def create_community_post(community_id):
    ...
    db.commit()
    db.close()
    flash('Пост опубликован в сообществе!')
...
def delete_community(community_id):
    ...
    c = db.cursor()
    is_admin = c.execute('SELECT * FROM chat_members WHERE chat_id=? AND user_id=? AND role="admin"',
                        (community_id, session['user_id'])).fetchone()
    if not is_admin:
        flash('Только создатель может удалить сообщество')
        return redirect(url_for('view_community', community_id=community_id))
    
    c.execute('DELETE FROM posts WHERE community_id=?', (community_id,))
    c.execute('DELETE FROM chat_members WHERE chat_id=?', (community_id,))
    c.execute('DELETE FROM chats WHERE id=?', (community_id,))
    db.commit()
    db.close()
