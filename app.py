...
def create_community():
    ...
    c.execute('INSERT INTO chats (name, description, is_group, is_community, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?)',
              (name, description, 1, 1, session['user_id'], datetime.now().isoformat()))
    chat_id = c.lastrowid
...
def profile():
    ...
    followers_count = c.execute('''SELECT COUNT(*) FROM friends 
                                   WHERE friend_id = ? AND status = 'pending'
                                   AND NOT EXISTS (
                                       SELECT 1 FROM friends f2 
                                       WHERE f2.user_id = ? AND f2.friend_id = friends.user_id AND f2.status = 'accepted'
                                   )''', (session['user_id'], session['user_id'])).fetchone()[0]
    
    following_count = c.execute('''SELECT COUNT(*) FROM friends 
                                   WHERE user_id = ? AND status = 'pending'
                                   AND NOT EXISTS (
                                       SELECT 1 FROM friends f2 
                                       WHERE f2.user_id = friends.friend_id AND f2.friend_id = ? AND f2.status = 'accepted'
                                   )''', (session['user_id'], session['user_id'])).fetchone()[0]
