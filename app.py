...
def friends():
    ...
    db = get_db()
    c = db.cursor()
    friends_list = c.execute('''SELECT u.id, u.username, u.avatar, u.bio 
    ...
    db.close()
    return render_template('friends.html', friends=friends_list, requests=requests, username=session.get('username', ''))
...
def search_users():
    ...
    users = c.execute('''SELECT id, username, avatar FROM users 
                         WHERE username LIKE ? AND id != ? LIMIT 10''', (f'%{query}%', session['user_id'])).fetchall()
    db.close()
...
def chat_view(chat_id):
    ...
    members = c.execute('''SELECT u.id, u.username, u.avatar, cm.role FROM users u
                           JOIN chat_members cm ON cm.user_id = u.id WHERE cm.chat_id = ?''', (chat_id,)).fetchall()
    db.close()
...
def create_group():
    ...
    db = get_db()
    c = db.cursor()
    c.execute('INSERT INTO chats (name, is_group, created_by, created_at) VALUES (?, ?, ?, ?)',
...
def communities():
    ...
    db = get_db()
    c = db.cursor()
    communities_list = c.execute('''SELECT c.*, 
                                   (SELECT COUNT(*) FROM chat_members WHERE chat_id = c.id) as members_count,
    ...
    db.close()
    return render_template('communities.html', communities=communities_list, username=session.get('username', ''))
...
def view_community(community_id):
    ...
    posts = c.execute('''SELECT p.*, u.username, u.avatar FROM posts p
                         JOIN users u ON p.user_id = u.id
                         WHERE p.community_id = ? ORDER BY p.created_at DESC''', (community_id,)).fetchall()
    members_count = c.execute('SELECT COUNT(*) FROM chat_members WHERE chat_id=?', (community_id,)).fetchone()[0]
